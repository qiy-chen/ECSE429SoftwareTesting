...
public class perf_todosCategoriesTest {
    ...
    public void addCategoryRelationTestPerf() throws Throwable{
        ...
        for (int i = 0; i < nb_objects_tested.length; i++){
            ...
            System.out.println(String.format("ADD %d CATEGORY RELATIONS:\t %d ms\t %f CPU \t %d Free Memory", nb_objects_tested[i], create_end_time - create_start_time, helper.getMeanListDouble(cpu_usage), helper.getMeanListLong(free_memory)));
            // Wait for server to be idle
            Thread.sleep(3000);
        }
    ...
    public void deleteCategoryRelationsTestPerf() throws Throwable {
        ...
        for (int i = 0; i < nb_objects_tested.length; i++){
            ...
            System.out.println(String.format("DELETE %d CATEGORY RELATIONS:\t %d ms\t %f CPU \t %d Free Memory", nb_objects_tested[i], delete_end_time - delete_start_time, helper.getMeanListDouble(cpu_usage_delete), helper.getMeanListLong(free_memory_delete)));
            // Wait for server to be idle
            Thread.sleep(3000);
        }
    }
