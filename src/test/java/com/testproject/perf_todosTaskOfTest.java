...
public class perf_todosTaskOfTest {
    static final String URLTodos = "http://localhost:4567/todos";
    static final String URLProjects = "http://localhost:4567/projects";
    ...
    public void cleanup() throws Throwable{
        ...
        // Cleanup projects
        HttpRequest test_request_proj = request.uri(new URI(URLProjects))
                                        .GET()
                                        .build();
        HttpResponse<String> response_proj = client.send(test_request_proj, BodyHandlers.ofString());
        @SuppressWarnings({ "unchecked" })
        List<LinkedTreeMap<String, Object>> proj_list= (List<LinkedTreeMap<String, Object>>) helper.jsonStringToMap(response_proj.body()).get(PROJECTS);
        // Cleanup all instances of projects
        for (int i = 0; i < proj_list.size(); i++){
            // Convert to hashmap
            @SuppressWarnings("unchecked")
            HashMap<String, Object> instance = new Gson().fromJson(new Gson().toJson(((LinkedTreeMap<String, Object>) proj_list.get(i))), HashMap.class);
            HttpRequest delete_request = request.uri(new URI(URLProjects + "/" + (String) instance.get("id") ))
                                .DELETE()
                                .build();
            client.send(delete_request, BodyHandlers.ofString());
        }
    ...
    @Test
    public void addProjectRelationsTestPerf() throws Throwable{
        ...
        for (int i = 0; i < nb_objects_tested.length; i++){
            ...
            String request_body = """
                    {title: "new proj"}
                    """;
            HttpRequest populate_request = request.uri(new URI(URLProjects))
            ...
            for (int j = 0; j < nb_objects_tested[i]; j ++){
                ...
                HttpRequest test_request = request.uri(new URI(URLTodos + "/" + test_instance.get("id") + "/tasksof"))
            ...
            System.out.println(String.format("ADD %d PROJECT RELATIONS:\t %d ms\t %f CPU \t %d Free Memory", nb_objects_tested[i], create_end_time - create_start_time, helper.getMeanListDouble(cpu_usage), helper.getMeanListLong(free_memory)));
            // Wait for server to be idle
            Thread.sleep(3000);
        }
    ...
    @Test
    public void deleteProjectRelationsTestPerf() throws Throwable {
        OperatingSystemMXBean operatingSystemMXBean = 
          (OperatingSystemMXBean) ManagementFactory.getOperatingSystemMXBean();
        for (int i = 0; i < nb_objects_tested.length; i++){
            // Populate server
            ...
            for (int j = 0; j < list_ids.size(); j ++){
                HashMap<String, Object> new_request_body = new HashMap<String, Object>();
                new_request_body.put("id", list_ids.get(j));
                HttpRequest test_request = request.uri(new URI(URLTodos + "/" + test_instance.get("id") + "/tasksof/" + list_ids.get(j)))
                                            .DELETE()
                                            .build();
                client.send(test_request, BodyHandlers.ofString());
                // Record cpu and memory stats
            ...
            System.out.println(String.format("DELETE %d PROJECT RELATIONS:\t %d ms\t %f CPU \t %d Free Memory", nb_objects_tested[i], delete_end_time - delete_start_time, helper.getMeanListDouble(cpu_usage_delete), helper.getMeanListLong(free_memory_delete)));
            // Wait for server to be idle
            Thread.sleep(3000);
        }
    }
